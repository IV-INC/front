      ...
      <div className="max-w-3xl mx-auto px-4 py-16">
        ...
        <div className="prose prose-invert prose-neutral max-w-none space-y-6 text-neutral-300">
          <p>
            ...
            <Link to="/terms" className="text-primary-400 hover:underline">Terms of Service</Link>). IV hosts user-generated content (UGC) but does not verify or guarantee the copyright validity of any submitted material. All responsibility for copyright compliance lies solely with the user. IV has no obligation to monitor or pre-screen any user-submitted logos, data, or other content for copyright infringement or any other issues. Submitted data is limited to company registration requirements, plus graphed information from Stripe Connect and Google Analytics 4 (GA4) integrations, which may be exposed to third parties (e.g., investors or authorized users via dashboards or visualizations). IV does not guarantee the authenticity of user-linked Stripe/GA4 data and assumes no liability for any third-party damages (e.g., investment losses) resulting from reliance on inaccurate or manipulated data:
          </p>

          <p>
            <strong>Company Basic Info:</strong> Company logo (square image recommended; user must confirm copyright ownership), company name (required), one-line description (10-100 characters), founding date (calendar selection), founding location (text or region selection), employee count (selection from '1~10', '10~100', '100~1000', '1000~10000', '10000+'), company introduction (1,000-10,000 characters), category (e.g., 'AI/ML', 'Fintech', 'Edtech', 'CleanTech'), maturity ('Pre-seed' to 'Series C+'), company links (website/GitHub/LinkedIn/X(Twitter)/YouTube; displayed as small icon buttons opening in new windows; IV does not verify linked content copyright).
          </p>

          <p>
            <strong>C-level Executive Input:</strong> CEO profile photo (recommended; user must confirm copyright), CEO introduction (text; required/recommended adjustable), CEO links (LinkedIn/X recommended), additional C-levels (name/title/profile photo optional/introduction optional/LinkedIn/X links optional/school/major optional; displayed in investor detail view right-side executive box with SNS icons linking externally).
          </p>

          <p>
            <strong>Other:</strong> Company introduction video (optional; user must confirm copyright), company news (optional), executive names/emails (optional; subject to{' '}
            <Link to="/privacy" className="text-blue-400 hover:underline">Privacy and Cookies Policy</Link>).
          </p>

          <p>
            <strong>Integrated Data Visualizations:</strong> Graphed information from Stripe Connect (e.g., read-only balances/transactions/payouts visualized as graphs/dashboards) and GA4 (e.g., traffic/user behavior/event metrics visualized); this data may be exposed to third parties (e.g., within the Services for analytics/insights, strictly as necessary and per user authorization; IV does not verify or assume liability for any copyrighted elements in graphed data).
          </p>
          ...
          <p>Description of the infringed work (e.g., original logo or data source).</p>
          <p>Location on IV Services (specific URL or company name).</p>
          <p>Your contact details (name, email, phone).</p>
          <p>Simple evidence of infringement (e.g., original link or proof of ownership). IV will act in its sole discretion, which may include content removal, account suspension, or termination. IV's decisions are final and incur no liability.</p>
          ...
          <p>
            IV does not encourage infringement and processes reports in good faith but bears no responsibility for outcomes. Policy changes posted at https://theIV.com; continued use constitutes agreement.
          </p>
        </div>
      </div>
    </div>
